...
A Python package for scraping, cleaning, and analyzing NFL draft data. Collects
big board rankings from multiple sources, merges them with actual draft picks,
and enriches the data with Approximate Value (AV) metrics from
...
- **Multi-source big board scraping** — collects prospect rankings from NFL Mock
  Draft Database (MDDB) and Jacklich10 (JLBB)
- **Fuzzy name matching** — reconciles player names across sources using
  `difflib` similarity matching
- **Combined rankings** — averages rankings from multiple big boards into a
  unified prospect ranking
- **Draft picks cleaning** — filters and normalizes raw draft pick data
- **Approximate Value enrichment** — retrieves AV stats via `sportsipy` with
  Pro Football Reference HTML fallback
- **Weighted career AV** — computes a time-discounted career AV giving 5% less
  weight to each subsequent year
- **Checkpoint/resume** — AV scraping saves periodic checkpoints to allow
...
Each pipeline stage can be run as a standalone script. All output CSVs are
...
Fuzzy-match and merge MDDB and JLBB rankings into a single combined board per
year:
...
Enrich draft picks with AV data from sportsipy/Pro Football Reference:
...
Each stage reads from the output of previous stages. Steps 1–2 can run in
parallel. Steps 4–5 can run independently of steps 1–3.
